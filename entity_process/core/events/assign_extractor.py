...
class AssignExtractor(BaseExtractor):
    def __init__(
    ...
    def get_grade_assignment_event_object(self, event, event_type: EventType):
        ...
        grade_qualifier = "Assigned grade"
        if event_type == EventType.UPDATED:
            event_qualifier = "evt_upt"
            grade_qualifier = "Updated grade"

        attributes = build_attributes(event, self.related_event_columns["log"])

        result = {
            "id": f"{event_qualifier}_grade_assignment_{event["id"]}",
            "type": f"{event_type.value.name}_grade_assignment",
            "time": format_date(event["timecreated"]),
            "attributes": attributes,
        }
