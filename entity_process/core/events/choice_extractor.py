...
from utils.extractor_utils import build_attributes, get_formatted_event_id, get_formatted_relationship, get_module_event_type_name
...
class ChoiceExtractor(BaseExtractor):
    ...
    def get_viewed_event_object(self, event, ids):
        ...
        result = {
            ...
            "type": get_module_event_type_name(
                self.object_type, EventType.VIEWED
            ),
    ...
    def get_deleted_module_event_object(self, event):
        ...
        if instance:
            ...
            if choice:
                course_relationships = self.get_course_relation(EventType.DELETED, choice["course"])
    ...
    def get_completed_module_event_object(self, event):
        ...
        if choice:
            course_relationships = self.get_course_relation(EventType.COMPLETED,choice["course"])
            if course_relationships:
                relationships.append(course_relationships)
    ...
    def get_create_choice_answer_event_object(self, event):
        attributes = build_attributes(
            event, self.related_event_columns["log"]
        )
        result = {
            "id": get_formatted_event_id(
